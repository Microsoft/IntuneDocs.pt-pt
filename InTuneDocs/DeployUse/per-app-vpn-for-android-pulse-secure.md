...
title: [VPN por aplicação para Android, utilizando Pulse Secure | Microsoft Intune]
description:
keywords:
author: [nbigman]
manager: [jeffgilb]
...
### Passo 1: criar um perfil da VPN

1. Na [consola de administração do Microsoft Intune](https://manage.microsoft.com), clique em **Política** > **Adicionar Política**.
2. Selecione um modelo para a nova política, expandindo **Android** e, em seguida, escolha **Perfil da VPN (Android 4 e posterior)**.

3. No modelo, para **Tipo de ligação**, escolha **Pulse Secure**.
4. Preencha e guarde o perfil da VPN. Para mais detalhes sobre perfis de VPN, consulte [Perfis de VPN](Help%20users%20connect%20to%20their%20work%20using%20VPN%20profiles%20with%20Microsoft%20Intune.md).

> [!NOTE]
Tome nota do nome do perfil da VPN para utilização no próximo passo. Por exemplo, **MyAppVpnProfile**.
   
### Passo 2: criar uma política personalizada
   ...
   4. Forneça um nome de definição.
...
Pode especificar que a sua lista de aplicações *não* irá ter permissão para utilizar a ligação VPN, utilizando o valor **BLACKLIST**.  Todas as outras aplicações estabelecerão ligação através de VPN.

Em alternativa, pode especificar que *apenas* as aplicações especificadas podem utilizar a ligação VPN, ao utilizar o valor **WHITELIST**.
 

1.  Em Definições OMA-URI, clique em **Adicionar**.
2.  Forneça um nome de definição.
...
Tem de implementar as *duas* políticas para os *mesmos* grupos do Intune.
