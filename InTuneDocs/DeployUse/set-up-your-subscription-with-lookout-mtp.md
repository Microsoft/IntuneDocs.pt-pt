...
Para preparar a sua subscrição de modo a integrar o serviço Lookout MTP, o suporte do Lookout (enterprisesupport@lookout.com) irá precisar das seguintes informações sobre o seu Azure Active Directory (Azure AD): 
...
Utilize as informações na secção seguinte para reunir as informações que deve fornecer à equipa de suporte do Lookout.  

## Obter informações do Azure AD
### Obter o ID de inquilino do Azure AD
Inicie sessão no portal de gestão do Azure AD: https://manage.windowsazure.com e selecione a sua subscrição. 

![captura de ecrã da página do Azure AD que apresenta o nome do inquilino](../media/mtp/aad_tenant_name.png) Ao escolher o nome da sua subscrição, o URL resultante incluirá o ID de subscrição.  Se tiver dificuldades em localizar o seu ID de subscrição, o [artigo de suporte da Microsoft](https://support.office.com/en-us/article/Find-your-Office-365-tenant-ID-6891b561-a52d-4ade-9f39-b492285e2c9b?ui=en-US&rs=en-US&ad=US) fornece sugestões adicionais.   
...
* **Acesso Restrito:** impede o acesso a vários módulos de configuração e inscrição da consola do Lookout MTP; permite o acesso só de leitura ao módulo **Política de Segurança** da consola do Lookout MTP.  
...
Poderá encontrar o **ID de Objeto de Grupo** na página de propriedades do grupo na consola de gestão do Azure AD.
...
Tem de utilizar uma conta com a função de Administrador Global do Azure AD ao iniciar sessão na consola do Lookout MTP pela primeira vez. O Lookout MTP requer que efetue esta ação de modo a registar o inquilino do Azure AD.   Este nível de privilégio no Azure AD não será exigido ao utilizador em inícios de sessão posteriores.  No primeiro início de sessão, será apresentada uma página de consentimento. Selecione **Aceitar** para concluir o registo.

![captura de ecrã da página de primeiro início de sessão da consola do Lookout MTP](../media/mtp/lookout_mtp_initial_login.png) Ao aceitar e dar o seu consentimento, será redirecionado para a Consola do Lookout MTP. Após o registo inicial, é possível efetuar inícios de sessão posteriores através do URL: https://aad.lookout.com
...
Os passos seguintes descrevem as tarefas que têm de ser realizadas para concluir a configuração do Lookout MTP na [Consola do Lookout MTP](https://aad.lookout.com).

### Passo 2: configurar o conector do Intune

Na consola do Lookout MTP, aceda ao módulo **Sistema**, selecione o separador **Conectores** e, em seguida, selecione **Intune**.
...
Na opção Definições da Ligação, configure a frequência do heartbeat em minutos.  Ao concluir este passo, o seu conector do Intune estará pronto.  
...
Na opção **Gestão de Inscrições**, defina um conjunto de utilizadores cujos dispositivos devem ser inscritos no Lookout.   Para tal, a melhor prática é começar por um grupo de utilizadores pequeno para testar e familiarizar-se com o processo de integração.  Quando estiver satisfeito com o resultado dos testes, pode alargar a inscrição a grupos de utilizadores adicionais.
...
* A criação de grupos dentro de outros grupos não é atualmente suportada.  Os grupos do Azure AD especificados devem conter apenas utilizadores e não grupos aninhados.
...
Se pretender receber alertas por e-mail relativos a ameaças, inicie sessão na [consola do Lookout MTP](https://aad.lookout.com)com a conta de utilizador que deve receber as notificações.  Aceda ao módulo **Sistema** e, no separador **Preferências**, selecione as notificações pretendidas e defina as mesmas como **ATIVADO**. Guarde as suas alterações.
