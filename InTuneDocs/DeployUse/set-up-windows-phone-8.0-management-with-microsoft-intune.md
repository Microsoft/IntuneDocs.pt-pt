...
title: Configurar a gestão do Windows Phone 8.0 com o Microsoft Intune | Microsoft Intune
description:
keywords:
author: NathBarn
manager: jeffgilb
...
Antes de poder gerir dispositivos móveis Windows Phone com o Intune, tem de configurar os requisitos de gestão. [Criar um CNAME DNS](set-up-windows-phone-management-with-microsoft-intune.md) ajuda os utilizadores a ligarem-se ao portal da empresa do Intune. O Windows Phone 8.0 requer um certificado da Symantec para estabelecer uma ligação IP encriptada entre os dispositivos e o Intune. Também é necessário um certificado para assinar aplicações de linha de negócio.
...
1.  **Configurar o Intune**
    Se ainda não o fez, prepare a gestão de dispositivos móveis [definindo a autoridade de gestão de dispositivos móveis](https://technet.microsoft.com/library/mt346013.aspx) como **Microsoft Intune** e configurando a MDM.
...
2.  **Definir um alias DNS para o endereço do servidor de inscrição** (opcional)
    ...
    2.  Introduza o URL do domínio verificado do site da empresa na caixa **Especificar um nome de domínio verificado** e, em seguida, clique em **Testar Deteção Automática**..
    ...
    3.  Crie registos de recursos DNS **CNAME** para o domínio da sua empresa. Os registos de recursos CNAME têm de conter as seguintes informações:
        ...
        |Nome de Anfitrião|Aponta para|TTL|
        |-------------|-------------|-------|
        |enterpriseenrollment.dominio_da_empresa.com|enterpriseenrollment.manage.microsoft.com |1 Hora|
        ...
        -   `manage.microsoft.com` – Suporta o redirecionamento para o serviço Intune com reconhecimento de domínio a partir do nome de domínio do e-mail
...
3.  **Gestão de certificados para suportar a assinatura de aplicações**
    [Necessário para dispositivos Windows Phone 8.0 e Windows Phone 8.1 que não conseguem aceder à Loja do Windows e/ou que precisem de aplicações de linha de negócio.]
    ...
    5.  **Exportar um certificado de assinatura**
        ...
        No **Assistente para Exportar Certificados**, selecione **Sim, exportar a chave privada** e, em seguida, clique em **Seguinte**. **Selecionar Personal Information Exchange – PKCS #12 (.PFX)** e selecione **Incluir todos os certificados no caminho de certificação se possível**. Conclua o assistente. Para mais informações, consulte [Como Exportar um Certificado com a Chave Privada](http://go.microsoft.com/fwlink/?LinkID=203031).
    ...
    6.  **Transferir e assinar a aplicação Portal da Empresa**
        ...
        1.  **Transferir o Portal da Empresa**
            Transfira o [Portal da Empresa do Intune para Windows Phone](http://go.microsoft.com/fwlink/?LinkId=268440) a partir do Centro de Transferências. A localização de instalação predefinida é `C:\Program Files (x86)\Microsoft Corporation\Windows Intune Company Portal for Windows Phone`.

        2.  **Transferir o Windows Phone 8.0 SDK**
            Transferir o [Windows Phone SDK](http://go.microsoft.com/fwlink/?LinkId=615570).

        3.  **Assinar com código a aplicação Portal da Empresa**
            Utilize a aplicação XAPSignTool transferida com o SDK para assinar o portal da empresa com o ficheiro .pfx que criou a partir do certificado da Symantec. Para mais informações, consulte [Como assinar uma aplicação da empresa com a XapSignTool](http://go.microsoft.com/fwlink/?LinkID=280195).

    7.  **Carregar a aplicação Portal da Empresa para o Intune**
        Carregue o ficheiro assinado da aplicação Portal da Empresa e o seu certificado de assinatura com código para disponibilizar a aplicação aos seus utilizadores finais.
...
4.  **Informar os utilizadores sobre como obter acesso aos recursos da empresa com o portal da empresa**
    Os seus utilizadores terão de saber como inscrever os respetivos dispositivos e o que esperar quando passarem a ser geridos. [O que dizer aos utilizadores finais sobre a utilização do Microsoft Intune](what-to-tell-your-end-users-about-using-microsoft-intune.md)
...
2.  **Transferir o Windows Phone SDK**
    Transfira o [Windows Phone SDK 8.0](http://go.microsoft.com/fwlink/?LinkId=615570) (http://go.microsoft.com/fwlink/?LinkId=268439) e instale-o no seu computador. Este SDK é necessário para gerar um token de inscrição de aplicações.

3.  **Gerar um ficheiro AETX**
    Gere um ficheiro de token de inscrição de aplicações (.aetx) a partir do ficheiro PFX da Symantec com o AETGenerator.exe, que faz parte do Windows Phone SDK 8.0. Para obter instruções sobre como criar um ficheiro AETX, consulte [How to generate an application enrollment token for Windows Phone](https://msdn.microsoft.com/library/windows/apps/jj735576.aspx)(Como gerar um token de inscrição de aplicações para Windows Phone)

4.  **Transferir o Windows SDK para Windows 8.1**
    Transfira e instale o [Windows Phone SDK](http://go.microsoft.com/fwlink/?LinkId=613525) (http://go.microsoft.com/fwlink/?LinkId=613525). Tenha em atenção que o script do PowerShell incluído com a aplicação Portal da Empresa utiliza a localização de instalação predefinida, `${env:ProgramFiles(x86)}\Windows Kits\8.1`. Se instalar noutra localização, tem de incluí-la num parâmetro de cmdlet.

5.  **Assinar a aplicação com código através do PowerShell**
    Na qualidade de administrador, abra o **Windows PowerShell** no computador anfitrião instalado com o Windows SDK, o Certificado de Assinatura de Código de Dispositivo Móvel Empresarial da Symantec, navegue para o ficheiro Sign-WinPhoneCompanyPortal.ps1 e execute o script.
