...
description: "Descreve como anular a inscrição de dispositivos Android no Intune se tiver recusado os termos de utilização e não conseguir iniciar sessão na aplicação Portal da Empresa"
keywords: 
author: staciebarker
ms.author: stabar
manager: angrobe
...
# Anular a inscrição do seu dispositivo do Intune caso tenha recusado os "Termos de Utilização"

A melhor forma de anular a inscrição do dispositivo Android é aceitar os termos de utilização, iniciar sessão na aplicação do Portal da Empresa e, em seguida, utilizar [estas instruções](unenroll-your-device-from-intune-android.md) para anular a inscrição. No entanto, se recusou os termos de utilização ao tentar iniciar sessão na aplicação Portal da Empresa, será impedido de iniciar sessão no Portal da Empresa em tentativas futuras. Neste caso, deve utilizar estas instruções de "solução" para anular a inscrição do seu dispositivo.
