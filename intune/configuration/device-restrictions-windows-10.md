...
- **Trusted app installation**: Choose if non-Microsoft Store apps can be installed, also known as sideloading. Sideloading is installing, and then running or testing an app that isn't certified by the Microsoft Store. For example, an app that is internal to your company only. As opções são:
...
- **Developer unlock**: Allow Windows developer settings, such as allowing sideloaded apps to be modified by end users. As opções são:
  - **Not configured** (default): Uses the OS default.
...
- **Apps from store only**: This setting determines the user experience when users install apps from places other than the Microsoft Store. As opções são:
...
- **Cellular data channel**: Choose if end users can use data, like browsing the web, when connected to a cellular network. As opções são:
...
- **Printer discovery URL**: Enter the URL for finding cloud printers. Por exemplo, introduza `https://cloudprinterdiscovery.contoso.com`.
- **Printer access authority URL**: Enter the authentication endpoint URL to get OAuth tokens. Por exemplo, introduza `https://azuretenant.contoso.com/adfs`.
- **Azure native client app GUID**: Enter the GUID of a client application allowed to get OAuth tokens from the OAuthAuthority. Por exemplo, introduza `E1CF1107-FF90-4228-93BF-26052DD2C714`.
- **Print service resource URI**: Enter the OAuth resource URI for print service configured in the Azure portal. Por exemplo, introduza `http://MicrosoftEnterpriseCloudPrint/CloudPrint`.
- **Maximum printers to query**: Enter the maximum number of printers that you want to be queried. The default value is `20`.
- **Printer discovery service resource URI**: Enter the OAuth resource URI for printer discovery service configured in the Azure portal. Por exemplo, introduza `http://MopriaDiscoveryService/CloudPrint`.
...
- **Turn on GDI scaling for apps**: **Add** the legacy apps that you want GDI DPI scaling turned on. Por exemplo, introduza: `filename.exe` ou `%ProgramFiles%\Path\Filename.exe`.

  GDI DPI scaling is turned on for all legacy applications in your list.

- **Turn off GDI scaling for apps**: **Add** the legacy apps that you want GDI DPI scaling turned off. Por exemplo, introduza: `filename.exe` ou `%ProgramFiles%\Path\Filename.exe`.
...
- **Ink Workspace**: Choose if and how user access the ink workspace. As opções são:
...
- **Automatic redeployment**: Choose **Allow** so users with administrative rights can delete all user data and settings using **CTRL + Win + R** at the device lock screen. The device is automatically reconfigured and re-enrolled into management. **Não configurado** (predefinição) impede esta funcionalidade.
...
- **Locked screen picture URL (desktop only)** : Enter the URL to a picture in JPG, JPEG, or PNG format that's used as the Windows lock screen wallpaper. Por exemplo, introduza `https://contoso.com/image.png`. This setting locks the image, and can't be changed afterwards.
...
The available settings change depending on what you choose. As opções são:
...
- **Start Microsoft Edge with**: Choose which pages open when Microsoft Edge starts. As opções são:
...
- **New Tab URL**: Enter the URL to open on the New Tab page. Por exemplo, introduza: `https://www.bing.com` ou `https://www.contoso.com`.

- **Home button**: Choose what happens when the home button is selected. As opções são:
  ...
  - **Home button URL**: Enter the URL to open. Por exemplo, introduza: `https://www.bing.com` ou `https://www.contoso.com`.
...
- **First Run Experience URL list location** (Windows 10 Mobile only): Enter the URL that points to the XML file containing the first run page URL(s). Por exemplo, introduza `https://www.contoso.com/sites.xml`.
...
- **Enterprise mode site list location** (Desktop only): Enter the URL that points to the XML file containing a list of web sites that open in Enterprise mode. Users can't change this list. Por exemplo, introduza `https://www.contoso.com/sites.xml`.
- **Message when opening sites in Internet Explorer**: Use this setting to configure Microsoft Edge to show a notification before a site opens in Internet Explorer 11. As opções são:
...
- **Show Favorites bar**: Choose what happens to the favorites bar on any Microsoft Edge page. As opções são:
...
- **Default search engine**: Choose the default search engine on the device. Os utilizadores finais podem alterar este valor em qualquer altura. As opções são:
  - Search engine in client Microsoft Edge settings
  - Bing
  - Google
  - Yahoo
  - Custom value: In **OpenSearch Xml URL**, enter an HTTPS URL with the XML file that includes the short name and the URL to the search engine, at minimum. Por exemplo, introduza `https://www.contoso.com/opensearch.xml`.
...
- **Sync browser settings between user's devices**: Choose how you want to sync browser settings between devices. As opções são:
...
- **Cookies**: Choose how cookies are handled in the web browser. As opções são:
...
- **Use manual proxy server**: Choose **Allow** to manually enter the  name or IP address, and TCP port number of a proxy server. **Not configured** (default) doesn't let you manually enter details of a proxy server.
  ...
  - **Proxy exceptions**: Enter any URLs that must not use the proxy server. Utilize um ponto e vírgula para separar cada item.
...
- **Password**: **Require** the end user to enter a password to access the device. **Not configured** (default) allows access to the device without a password. Applies to local accounts only. Domain account passwords remain configured by Active Directory (AD) and Azure AD.
  ...
  - **Required password type**: Choose the type of password. As opções são:
  ...
  - **Simple passwords**: Set to **Block** so users can't create simple passwords, such as `1234` or `1111`. Set to **Not configured** (default) to let users create passwords like `1234` or `1111`. Esta definição também permite ou bloqueia a utilização de palavras-passe por imagem do Windows.
...
- **Web sign-in**: Enables Windows sign in support for non-ADFS (Active Directory Federation Services) federated providers, such as Security Assertion Markup Language (SAML). SAML uses secure tokens that provide web browsers a single sign-on (SSO) experience. As opções são:
...
- **Radios**: Some apps use radios (for example, Bluetooth) in your device to send and receive data and need to turn these radios on or off. Defina se esta aplicação pode controlar estes rádios.
...
- **Desktop background picture URL (Desktop only)** : Enter the URL to a picture in .jpg, .jpeg or .png format that you want to use as the Windows desktop wallpaper. Os utilizadores não podem alterar a imagem. Por exemplo, introduza `https://contoso.com/logo.png`.
...
- **Share usage data**: Choose the level of diagnostic data that's submitted. As opções são:
...
- **Send Microsoft Edge browsing data to Microsoft 365 Analytics**: To use this feature, set the **Share usage data** settings to **Enhanced** or **Full**. This feature controls what data Microsoft Edge sends to Microsoft 365 Analytics for enterprise devices with a configured commercial ID. As opções são:
...
- **Telemetry proxy server**: Enter the fully qualified domain name (FQDN) or IP address of a proxy server to forward Connected User Experiences and Telemetry requests, using a Secure Sockets Layer (SSL) connection. O formato para esta definição é *servidor*:*porta*. If the named proxy fails, or if a proxy isn't entered when enabling this policy, the Connected User Experiences and Telemetry data isn't sent, and stays on the local device.
...
- **Safe Search (mobile only)** : Control how Cortana filters adult content in search results. As opções são:
...
- **Most used apps**: **Block** hides the most used apps from showing on the start menu. Também desativa o seletor correspondente na aplicação Definições. **Not configured** (default) shows the most used apps.
- **Recently added apps**: **Block** hides recently added apps from showing on the start menu. Também desativa o seletor correspondente na aplicação Definições. **Not configured** (default) shows the recently added apps on the start menu.
- **Start screen mode**: Choose how the start screen is shown. As opções são:
...
- **Recently opened items in Jump Lists**: **Block** hides recent jump lists from being shown on the start menu and taskbar. Também desativa o seletor correspondente na aplicação Definições. **Not configured** (default) shows recently opened items in the jumplists.
- **App list**: Choose how the all apps lists are shown. As opções são:
...
- **Documents on Start**: Hide or show the Documents folder in the Windows Start menu. As opções são:
...
- **Downloads on Start**: Hide or show the Downloads folder in the Windows Start menu. As opções são:
  ...
  - **Hide**: The shortcut is hidden, and disables the setting in the Settings app.
  - **Show**: The shortcut is shown, and disables the setting in the Settings app.
- **File Explorer on Start**: Hide or show File Explorer in the Windows Start menu. As opções são:
  - **Not configured** (default): No setting is forced. Users choose to show or hide the shortcut.
  - **Hide**: The shortcut is hidden, and disables the setting in the Settings app.
  - **Show**: The shortcut is shown, and disables the setting in the Settings app.
- **HomeGroup on Start**: Hide or show the HomeGroup shortcut in the Windows Start menu. As opções são:
  - **Not configured** (default): No setting is forced. Users choose to show or hide the shortcut.
  - **Hide**: The shortcut is hidden, and disables the setting in the Settings app.
  - **Show**: The shortcut is shown, and disables the setting in the Settings app.
- **Music on Start**: Hide or show the Music folder in the Windows Start menu. As opções são:
  - **Not configured** (default): No setting is forced. Users choose to show or hide the shortcut.
  - **Hide**: The shortcut is hidden, and disables the setting in the Settings app.
  - **Show**: The shortcut is shown, and disables the setting in the Settings app.
- **Network on Start**: Hide or show Network in the Windows Start menu. As opções são:
  - **Not configured** (default): No setting is forced. Users choose to show or hide the shortcut.
  - **Hide**: The shortcut is hidden, and disables the setting in the Settings app.
  - **Show**: The shortcut is shown, and disables the setting in the Settings app.
- **Personal folder on Start**: Hide or show Personal folder in the Windows Start menu. As opções são:
  - **Not configured** (default): No setting is forced. Users choose to show or hide the shortcut.
  - **Hide**: The shortcut is hidden, and disables the setting in the Settings app.
  - **Show**: The shortcut is shown, and disables the setting in the Settings app.
- **Pictures on Start**: Hide or show the folder for pictures in the Windows Start menu. As opções são:
  - **Not configured** (default): No setting is forced. Users choose to show or hide the shortcut.
  - **Hide**: The shortcut is hidden, and disables the setting in the Settings app.
  - **Show**: The shortcut is shown, and disables the setting in the Settings app.
- **Settings on Start**: Hide or show the Settings shortcut in the Windows Start menu. As opções são:
  - **Not configured** (default): No setting is forced. Users choose to show or hide the shortcut.
  - **Hide**: The shortcut is hidden, and disables the setting in the Settings app.
  - **Show**: The shortcut is shown, and disables the setting in the Settings app.
- **Videos on Start**: Hide or show the folder for videos in the Windows Start menu. As opções são:
  - **Not configured** (default): No setting is forced. Users choose to show or hide the shortcut.
  - **Hide**: The shortcut is hidden, and disables the setting in the Settings app.
  - **Show**: The shortcut is shown, and disables the setting in the Settings app.
...
- **Windows Spotlight**: **Block** turns off Windows spotlight on the lock screen, Windows Tips, Microsoft consumer features, and other related features. If your goal is to minimize network traffic from devices, set this to **Block**. **Not configured** (default) allows Windows spotlight features and may be controlled by end users. When enabled, you can also allow or block the following settings:
  ...
  - **Consumer Features**: **Block** turns off experiences that are typically for consumers only, such as start suggestions, membership notifications, post-out of box experience app installation, and redirect tiles. **Não configurado** (predefinição) permite estas funcionalidades.
...
- **Network Inspection System (NIS)** : NIS helps to protect devices against network-based exploits. Utiliza as assinaturas de vulnerabilidades conhecidas do Microsoft Endpoint Protection Center para ajudar a detetar e bloquear tráfego malicioso.
...
- **Security intelligence update interval (in hours)** : Enter the interval that Defender checks for new security intelligence, from 0-24. As opções são:
...
- **Monitor file and program activity**: Allows Defender to monitor file and program activity on devices. As opções são:
...
- **Prompt users before sample submission**: Controls whether potentially malicious files that might require further analysis are automatically sent to Microsoft. As opções são:
...
- **Type of system scan to perform**: Schedule a system scan, including the level of scanning, and the day and time to run the scan. As opções são:
...
- **Detect potentially unwanted applications**: Choose the level of protection when Windows detects potentially unwanted applications. As opções são:
...
- **Actions on detected malware threats**: Choose how you want to handle malware threads. **Not configured** (default) lets Microsoft Defender choose the best option. When set to **Enable**, choose the actions you want Defender to take for each threat level it detects: low, moderate, high, and severe. As opções são:
...
- **Files and folders to exclude from scans and real-time protection**: Adds one or more files and folders like **C:\Path** or **%ProgramFiles%\Path\filename.exe** to the exclusions list. Estes ficheiros e pastas não são incluídos em análises em tempo real ou agendadas.
