...
1. Na pasta **NDESConnectorSvc** , abra o arquivo **NDESConnector. exe. config** no bloco de notas.
   ...
   a. Atualize o valor da chave de `RACertThumbprint` com o valor de impressão digital do certificado que você copiou na seção anterior. Por exemplo:
...
2. Criar um perfil de certificado confiável no portal do Intune:
   ...
   a. Entre no centro de [Administração do Microsoft Endpoint Manager](https://go.microsoft.com/fwlink/?linkid=2109431).
   ...
   d. Selecione **configurações**e, em seguida, navegue até o arquivo. cer do certificado de AC raiz confiável que você exportou para uso com este perfil de certificado e, em seguida, selecione **OK**.
...
4. No painel **certificado PKCS** , configure os parâmetros com os valores da tabela a seguir. Esses valores são necessários para emitir certificados PKCS de uma AC DigiCert, por meio do Intune Certificate Connector.
...
5. Conclua a configuração do perfil para atender às suas necessidades de negócios e, em seguida, selecione **criar** para salvar o perfil.

6. Na página *visão geral* do novo perfil, selecione **atribuições** e configure um grupo apropriado que receberá esse perfil. Pelo menos um utilizador ou dispositivo tem de fazer parte do grupo atribuído.
