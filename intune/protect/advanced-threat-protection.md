...
- Um ataque de privilégio elevado é iniciado, e um invasor de um computador remoto tem direitos de administrador para o dispositivo da vítima.  
- O invasor então acessa remotamente os outros dispositivos do usuário. Essa violação de segurança pode afetar toda a organização.  
...
- Microsoft Intune ambiente, com dispositivos Windows 10 [gerenciados pelo Intune](../enrollment/windows-enroll.md) que também são ingressados no Azure AD
...
4. Na **central de segurança do Microsoft defender**:
    ...
    2. Para **Microsoft Intune conexão**, escolha **ativado**:
...
Normalmente, você faz essa tarefa uma vez. Depois de habilitar o Microsoft defender ATP para seu locatário do Intune, você não precisará fazê-lo novamente.
...
Depois de carregar um dispositivo usando o pacote de configuração, você não precisará fazê-lo novamente. Você também pode integrar dispositivos usando uma [política de grupo ou System Center Configuration Manager (SCCM)](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-atp/configure-endpoints).
...
6. Defina as configurações:
   ...
   - **Tipo de pacote de configuração de cliente do Microsoft defender ATP**: selecione **carregar** para adicionar o pacote de configuração ao perfil. Selecione **transferir** para remover o pacote de configuração do perfil.
...
7. Selecione **OK**e **criar** para salvar suas alterações, que cria o perfil.
...
5. Nas configurações do **Microsoft defender ATP** , defina **exigir que o dispositivo esteja em ou sob a pontuação de risco do computador** para seu nível preferido. 
   ...
   - **Clear**: esse nível é o mais seguro. O dispositivo não pode ter nenhuma ameaça existente e ainda acessar os recursos da empresa. Se alguma ameaça for encontrada, o dispositivo será avaliado como não compatível. (Os usuários do Microsoft defender ATP têm o valor *seguro*.)
   - **Baixo**: o dispositivo estará em conformidade se apenas ameaças de nível baixo existirem. Dispositivos com níveis de ameaça médio ou alto não são compatíveis.
   - **Médio**: o dispositivo estará em conformidade se as ameaças encontradas no dispositivo forem baixas ou médias. Se forem detectadas ameaças de alto nível, o dispositivo será determinado como não compatível.
...
> [!TIP]  
> O acesso condicional é uma tecnologia Azure Active Directory (Azure AD). O nó de acesso condicional acessado do *Intune* é o mesmo nó acessado do *Azure ad*.  

1. Entre no [Intune](https://go.microsoft.com/fwlink/?linkid=2090973) e selecione **acesso condicional** > **nova política**.
2. Insira um **nome**de política e selecione **usuários e grupos**. Use as opções incluir ou excluir para adicionar seus grupos à política e selecione **concluído**.
3. Selecione **aplicativos de nuvem**e escolha quais aplicativos proteger. Por exemplo, escolha **selecionar aplicativos**e selecione **Office 365 SharePoint online** e **Office 365 Exchange Online**.

    Selecione **concluído** para salvar as alterações.

4. Selecione **condições** > **aplicativos cliente** para aplicar a política a aplicativos e navegadores. Por exemplo, selecione **Sim**e, em seguida, habilitar **navegador** e **aplicativos móveis e clientes de desktop**.

    Selecione **concluído** para salvar as alterações.

5. Selecione **conceder** para aplicar o acesso condicional com base na conformidade do dispositivo. Por exemplo, selecione **conceder acesso** > **exigir que o dispositivo seja marcado como compatível**.

    Escolha **selecionar** para salvar as alterações.

6. Selecione **habilitar política**e **criar** para salvar suas alterações.
...
1. Entre no [Intune](https://go.microsoft.com/fwlink/?linkid=2090973).
2. Selecione **conformidade do dispositivo** > **conformidade de política**.
