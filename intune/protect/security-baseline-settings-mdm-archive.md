...
## <a name="preview-mdm-security-baseline-for-october-2018"></a>Visualização: linha de base de segurança do MDM para outubro de 2018  
...
- **Nível de autenticação**  
  ...
  - *Enviar respostas LM e NTLM*: os clientes usam autenticação LM e NTLM e nunca usam a segurança de sessão NTLMv2; os controladores de domínio aceitam autenticação LM, NTLM e NTLMv2. 
...
- **Comportamento da solicitação de elevação do administrador**  
  ...
  - *Solicitar credenciais na área de trabalho segura*: quando uma operação requer elevação de privilégio, o usuário recebe uma solicitação na área de trabalho segura para inserir um nome de usuário e senha com privilégios. Se o usuário inserir credenciais válidas, a operação continuará com o privilégio mais alto disponível do usuário. 
...
- **Exigir o modo de aprovação de administrador para administradores**  
  ...
  - *Não configurado*: o modo de aprovação de administrador e todas as configurações de política de UAC relacionadas estão desabilitadas. Observação: se essa configuração de política estiver desabilitada, a central de segurança o notificará de que a segurança geral do sistema operacional foi reduzida. 
...
- **Impedir enumeração anônima de contas SAM**  
  ...
  - *Não configurado*: nenhuma restrição adicional. Conte com as permissões padrão.  
...
- **Usar o modo de aprovação de administrador**  
  ...
  - *Não configurado*: a conta de administrador interna executa todos os aplicativos com privilégio administrativo completo.  
...
- **Permitir aplicativos de acesso à interface do usuário para locais seguros**  
  ...
  - *Sim*: programas UIA, incluindo a assistência remota do Windows, desabilitam automaticamente a área de trabalho segura para solicitações de elevação. Se você não desabilitar a configuração de política "controle de conta de usuário: alternar para a área de trabalho segura ao solicitar elevação", os prompts aparecerão na área de trabalho do usuário interativo em vez da área de trabalho segura. 
...
- **Inicialização do driver iniciar inicialização do sistema**  
  ...
  - *Inadequado, mas necessário para a inicialização*: o driver foi identificado como malware, mas o computador não pode ser inicializado com êxito sem carregar esse driver. 
