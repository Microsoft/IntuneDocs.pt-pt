...
3. Selecione o nó **modelos de certificado** , clique em **ação** > **gerenciar**.
...
2. Defina as seguintes configurações nas guias especificadas do modelo:
   ...
   - **Nome da entidade**:  
     ...
     ![Modelo, guia nome da entidade](./media/certificates-scep-configure/scep-ndes-subject-name.jpg)
   - **Extensões**:  
     - Verifique se a **Descrição das políticas de aplicativo** inclui a **autenticação de cliente**.  
       > [!IMPORTANT]  
       > Adicione apenas as políticas de aplicativo que você precisa. Confirme suas escolhas com os administradores de segurança.
     ...
     ![Modelo, guia extensões](./media/certificates-scep-configure/scep-ndes-extensions.jpg)  
   ...
   - **Segurança**:  
     ...
     ![Modelo, guia Segurança](./media/certificates-scep-configure/scep-ndes-security.jpg)  

   - **Tratamento de solicitação**:  
      A imagem a seguir é um exemplo. Sua configuração pode variar.  

     ![Modelo, guia tratamento de solicitação](./media/certificates-scep-configure/scep-ndes-request-handling.png) 

   - **Requisitos de emissão**:  
     A imagem a seguir é um exemplo. Sua configuração pode variar.  

     ![Modelo, guia requisitos de emissão](./media/certificates-scep-configure/scep-ndes-issuance-reqs.jpg)  
...
Por padrão, o Intune usa o valor configurado no modelo. No entanto, você pode configurar a AC para permitir que o solicitante Insira um valor diferente, e esse valor pode ser definido de dentro do console do Intune.  
...
2. Na AC emissora, use o snap-in autoridade de certificação para publicar o modelo de certificado. Selecione o nó **modelos de certificado** , selecione **ação** > **novo**modelo de certificado  > **a ser emitido**e, em seguida, selecione o modelo de certificado que você criou na seção anterior.  
...
1. No servidor que hospedará seu serviço de NDES, entre como um **administrador corporativo**e, em seguida, use o [Assistente para adicionar funções e recursos](https://docs.microsoft.com/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/hh831809(v=ws.11)) para instalar o NDES:
   ...
   1. No assistente, selecione **Active Directory serviços de certificados** para obter acesso aos serviços de função do AD CS. Selecione **serviço de registro de dispositivo de rede**, desmarque **autoridade de certificação**e conclua o assistente.  

      > [!TIP]  
      > Em **andamento da instalação**, não selecione **fechar**. Em vez disso, selecione o link **configurar Active Directory serviços de certificados no servidor de destino** . O assistente de **configuração do AD CS** é aberto, que você usa para o próximo procedimento neste artigo, *Configurar o serviço de NDES*. Após a abertura da configuração do AD CS, você pode fechar o assistente para adicionar funções e recursos.  
   ...
   2. Quando o NDES é adicionado ao servidor, o assistente também instala o IIS. Confirme se o IIS tem as seguintes configurações:  
      ...
      - **Filtragem de solicitações** de @no__t de**segurança** **do servidor Web** > -3  
      - **Desenvolvimento de aplicativos** >  **do servidor Web** > **ASP.net 3,5**  

        Instalar o ASP.NET 3,5 instala .NET Framework 3,5. Ao instalar o .NET Framework 3,5, instale o recurso Core **.NET Framework 3,5** e a **ativação http**.  
      - **Desenvolvimento de aplicativos** >  **do servidor Web** > **ASP.NET 4,5**  

        Instalar o ASP.NET 4,5 instala .NET Framework 4,5. Ao instalar o .NET Framework 4,5, instale o recurso Core **.NET Framework 4,5** , **ASP.NET 4,5**e o recurso de**ativação http** **dos serviços WCF** > .  

      - **Ferramentas de gerenciamento** > **compatibilidade de gerenciamento do IIS 6** >  compatibilidade de metabase do**IIS 6**  
      - **Ferramentas de gerenciamento** > **compatibilidade de gerenciamento do IIS 6** >  compatibilidade com WMI do**IIS 6**  
      - No servidor, adicione a conta de serviço de NDES como um membro do grupo **IIS_IUSR** local.  
...
1. No computador que hospeda o serviço de NDES, abra o assistente de **configuração do AD CS** e faça as seguintes atualizações:  
   ...
   > [!TIP]  
   > Se você estiver continuando no último procedimento e clicou no link **configurar Active Directory serviços de certificados no servidor de destino** , esse assistente já deve estar aberto. Caso contrário, abra Gerenciador do Servidor para acessar a configuração pós-implantação para Active Directory serviços de certificados.  
...
2. Depois que o assistente for concluído, atualize a seguinte chave do registro no computador que hospeda o serviço de NDES:  
   ...
   A tabela a seguir mapeia a finalidade do modelo de certificado para os valores no registro:
   
   |Finalidade do modelo de certificado (na guia tratamento de solicitação)|Valor do registro a ser editado|Valor visto no console de administração do Intune para o perfil SCEP|
   |------------------------|-------------------------|---|
   |Signature               |SignatureTemplate        |Assinatura digital |
   |Encriptação              |EncryptionTemplate       |Codificação de chave  |
   |Assinatura e criptografia|GeneralPurposeTemplate   |Codificação de chave<br/>Assinatura digital |  

   Por exemplo, se a finalidade do modelo de certificado for **criptografia**, edite o valor de **encryptiontemplate** como o nome do seu modelo de certificado.  
...
2. Associar o certificado de autenticação de servidor no IIS:  
   ...
   1. Depois de instalar o certificado de autenticação de servidor, abra o **Gerenciador do IIS**e selecione o **site da Web padrão**. No painel **ações** , selecione **associações**.  
   1. Selecione **Adicionar**, definir **tipo** como **https**e confirme se a porta é **443**.  
   1. Para **certificado SSL**, especifique o certificado de autenticação de servidor.  
 
3. No seu servidor de NDES, solicite e instale um certificado de **autenticação de cliente** de sua autoridade de certificação interna ou uma autoridade de certificação pública.  
...
3. Baixe e salve o conector para o arquivo SCEP. Salve-o em um local acessível do servidor no qual você vai instalar o conector.

   ![ConnectorDownload](./media/certificates-scep-configure/download-certificates-connector.png)

4. Após a conclusão do download, vá para o servidor que hospeda a função do serviço de registro de dispositivo de rede (NDES). Em seguida:  
   ...
   2. Execute o instalador (**NDESConnectorSetup. exe**). O instalador também instala o módulo de política para NDES e o serviço Web do CRP (ponto de registro de certificado) do IIS. O serviço Web do CRP, *CertificateRegistrationSvc*, é executado como um aplicativo no IIS.  
      ...
      - Quando você instala o NDES para o Intune autônomo, o serviço CRP é instalado automaticamente com o conector de certificado. 
...
5. Quando for solicitado o certificado do cliente para o conector de certificado, escolha **selecionar**e selecione o certificado de **autenticação de cliente** que você instalou em seu servidor NDES durante a etapa #3 do procedimento [instalar e associar certificados no servidor que hospeda o NDES](#install-and-bind-certificates-on-the-server-that-hosts-ndes) anteriormente neste artigo.  
   ...
   Depois de selecionar o certificado de autenticação de cliente, você será devolvido ao **certificado do cliente para Microsoft Intune Certificate Connector** superfície. Embora o certificado selecionado não seja mostrado, selecione **Avançar** para exibir as propriedades desse certificado. Selecione **Avançar**e **instalar**.

6. Após a conclusão do assistente, mas antes de fechar o assistente, **inicie a interface do usuário do conector de certificado**.  
...
7. Na interface do usuário do **conector de certificado** :  
   1. Selecione **entrar**e insira suas credenciais de administrador de serviço do Intune ou credenciais para um administrador de locatários com a permissão de administração global.  
   2. A conta usada deve ser atribuída a uma licença válida do Intune.  
   3. Depois de entrar, o conector de certificado do Intune baixa um certificado do Intune. Esse certificado é usado para autenticação entre o conector e o Intune. Se a conta usada não tiver uma licença do Intune, o conector (NDESConnectorUI. exe) falhará ao obter o certificado do Intune.  

      Se sua organização usa um servidor proxy e o proxy é necessário para que o servidor NDES acesse a Internet, selecione **usar servidor proxy**. Em seguida, insira o nome do servidor proxy, a porta e as credenciais da conta para se conectar.  

    4. Selecione a guia **avançado** e, em seguida, insira as credenciais para uma conta que tenha a permissão **emitir e gerenciar certificados** em sua autoridade de certificação emissora. **Aplique** suas alterações.  

    5. Agora você pode fechar a interface do usuário do conector de certificado.  

8. Abra um prompt de comando, digite **Services. msc**e, em seguida, **digite**. Clique com o botão direito do mouse no **serviço do conector do Intune** > **reiniciar**.


Para validar que o serviço está em execução, abra um navegador e insira a URL a seguir. Ele deve retornar um erro **403** : `https://<FQDN_of_your_NDES_server>/certsrv/mscep/mscep.dll`  

> [!NOTE]  
> O conector de certificado do Intune dá suporte a TLS 1,2. Se o servidor que hospeda o conector oferecer suporte a TLS 1,2, o TLS 1,2 será usado. Se o servidor não oferecer suporte a TLS 1,2, o TLS 1,1 será usado. Atualmente, o TLS 1,1 é usado para autenticação entre os dispositivos e o servidor.
