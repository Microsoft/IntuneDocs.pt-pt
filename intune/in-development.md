...
Para ver as configurações atuais, vá para [configurações de Wi-Fi do Android](wi-fi-settings-android.md) ou [configurações de Wi-Fi para Android Enterprise](wi-fi-settings-android-enterprise.md).
...
Para ver uma visão geral de todas as configurações que você pode definir, consulte [aplicar recursos e configurações em seus dispositivos usando perfis de dispositivo no Microsoft Intune](device-profiles.md).
