---
title: Configurar e gerir certificados SCEP com o Microsoft Intune
description: Saiba como configurar a sua infraestrutura e, em seguida, criar e atribuir perfis de certificado SCEP (Simple Certificate Enrollment Protocol) no Microsoft Intune.
keywords: ''
author: dougeby
ms.author: dougeby
manager: dougeby
...
---
# <a name="configure-and-manage-scep-certificates-with-microsoft-intune"></a>Configurar e gerir certificados SCEP com o Microsoft Intune
...
2.  Na AC emissora, utilize o snap-in Modelos de Certificado para criar um novo modelo personalizado ou copie um modelo existente, em seguida, edite um modelo existente (como o Modelo de Utilizador) para utilizar com o NDES.
    ...
    -   No separador **Segurança**, adicione a conta de serviço do NDES e conceda-lhe permissões de **Inscrição** no modelo. Os administradores do Intune que criam perfis SCEP precisam de direitos de **Leitura** para poderem navegar para o modelo ao criarem perfis de SCEP.
...
Para configurar a AC para permitir ao autor do pedido especificar o período de validade:
...
2. Na AC emissora, utilize o snap-in Autoridade de Certificação para publicar o modelo de certificado.
    Selecione o nó **Modelos de Certificados**, clique em **Ação**-&gt; **Novo** &gt; **Modelo de certificado a emitir** e, em seguida, selecione o modelo que criou no passo 2.
...
- Configurar o SPN para a conta do Serviço do NDES =======
-   Adicionar o NDES a um Windows Server e configurar ISS para suportar o NDES
>>>>>>> principal de 3365d47d4… Unir o ramo "release-intune-1803" à versão principal
...
-   Configure o SPN para a conta do Serviço do NDES
   ...
   1.  No servidor que aloja o NDES, inicie sessão como **Administrador de Empresa** e, em seguida, utilize o [Assistente para Adicionar Funções e Funcionalidades](https://technet.microsoft.com/library/hh831809.aspx) para instalar o NDES:
    ...
    2.  Quando o NDES é adicionado ao servidor, o assistente também instala o IIS. Certifique-se de que o IIS tem as seguintes configurações:
        ...
        -   **Servidor Web** &gt; **Programação de Aplicações** &gt; **ASP.NET 3.5**. A instalação do ASP.NET 3.5 instala o .NET Framework 3.5. Ao instalar o .NET Framework 3.5, instala tanto a funcionalidade **.NET Framework 3.5** principal como a **Ativação HTTP**.

        -   **Servidor Web** &gt; **Programação de Aplicações** &gt; **ASP.NET 4.5**. A instalação do ASP.NET 4.5 instala o .NET Framework 4.5. Ao instalar o .NET Framework 4.5, instale a funcionalidade **.NET Framework 4.5** principal, o **ASP.NET 4.5** e a funcionalidade **Serviços do WCF** &gt; **Ativação HTTP**.
...
1.  No Servidor do NDES, abra o assistente de Configuração de AD CS e, em seguida, faça as seguintes configurações:
...
3. O servidor do NDES recebe URLs extensos (consultas) que requerem a adição de duas entradas de registo:
...
4. No Gestor do IIS, selecione **Site Predefinido** -> **Filtragem de Pedidos** -> **Editar Definição de Funcionalidade** e altere o **Comprimento Máximo do URL** e a **Cadeia máxima de consulta** para *65534*, conforme mostrado.
...
2.  Clique em **Editar Definições de Funcionalidade** e, em seguida, defina os seguintes valores:
...
1. No portal do Azure, selecione a carga de trabalho **Configuração do dispositivo**.
...
5. Na lista pendente **Plataforma**, selecione a plataforma do dispositivo para este certificado SCEP. Atualmente, pode selecionar uma das seguintes plataformas para definições de restrição de dispositivos:
    - **Android**
    - **iOS**
    - **macOS**
    - **Windows Phone 8.1**
...
6. Na lista pendente **Tipo de perfil**, selecione **Certificado SCEP**.
7. No painel **Certificado SCEP**, configure as seguintes definições:
    ...
    - **Fornecedor de armazenamento de chaves (KSP)** (Windows Phone 8.1, Windows 8.1, Windows 10) – especifique onde é armazenada a chave do certificado. Escolha um dos seguintes valores:
    ...
    - **Certificado de Raiz** – Escolha um perfil de certificado da AC de raiz que tenha configurado anteriormente e atribuído ao utilizador ou dispositivo. Este certificado da AC tem de ser o certificado de raiz da AC que emite o certificado que está a configurar neste perfil de certificado. 
    - **Utilização da chave expandida** – selecione **Adicionar** para adicionar valores ao objetivo do certificado. Na maioria dos casos, o certificado exige a **Autenticação de Cliente** para o utilizador ou dispositivo poder ser autenticado num servidor. Contudo, pode adicionar mais utilizações de chave conforme necessário. 
    - **Definições de Inscrição**
        ...
        - **URLs do Servidor SCEP** – especifique um ou mais URLs para os servidores do NDES que emitem os certificados através de SCEP. 
8. Selecione **OK** e, em seguida, volte ao painel **Criar perfil** e selecione **Criar**.
